...
<script>
    $(document).ready(function(){
        $('[id^="title"]').bind('keyup', function(){
            //linkGen($(this).val());
        });
        $('[id^="title"]').bind('keypress', function(){
           // linkGen($(this).val());
        });
        $('[id^="title"]').bind('keydown', function(){
          //  linkGen($(this).val());
        });
    ...
    function linkGen(text){
        if(text != ''){
            var linkValue = textToTranslit(text);
            console.log(linkValue);
            linkValue = linkValue.replace(/[^A-Za-z0-9]/g,'_');
            $('#link').val(linkValue+'.html');
        } else{
           $('#link').val('');
        }
    }
...
<div class="manager-content">
    ...
    <form action="/manager/pages/create/" method="post" class="form">
